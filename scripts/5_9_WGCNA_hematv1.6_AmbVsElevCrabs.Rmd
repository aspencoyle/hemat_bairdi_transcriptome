---
title: "5_9_WGCNA_hematv1.6_AmbVsElevCrabs"
author: "Aidan Coyle"
...
We will include transcripts from only Hematodinium by using kallisto alignments to a transcriptome filtered to only include Alveolata sequences
...
Not including SC in this WGCNA, as only 1 elevated-temp crab has it

We'll also choose to scale and center CW, as numeric variables with means or SDs > 5 can induce collinearity with the intercept. 
...
# binarizeCategoricalVariable is because WGCNA doesn't play well with categorical variables. This turns it into a series of Y/N
# minCount is usually set at 3 - variables that don't appear 3+ times are excluded from the binarization unless otherwise specified
crabTraits <- data.frame("crab" = binarizeCategoricalVariable(x = c(rep(c("A", "B", "C", "G", "H", "I"), times = 2), "A", "B", "C"),
                         ...
                         "temp" = c(rep("Ambient", times = 9),
                                    ...
                                    rep("Ambient", times = 3)),
                         "hemat_level" = rep(c("L", "H", "H"), times = 5),
                         "CW" = scale(c(rep(c(132.2, 128.8, 145, 
                                              95.7, 111.8, 133.7), 
                                            times = 2),
                                        132.2, 128.8, 145), center = TRUE))
...
# If treating elevated day 0 as ambient
crabClinicalData <- data.frame("crab" = binarizeCategoricalVariable(x = c(rep(c(1, 2, 3, 4, 5, 6), times = 2), 1, 2, 3),
                                                              includePairwise = FALSE,
                                                              includeLevelVsAll = TRUE,
                                                              minCount = 2),
                         "day" = c(rep(0, times = 6),
                         ...
                         "temp" = c(rep(1, times = 9),
                                    rep(2, times = 3),
                                    rep(1, times = 3)),
                         "hemat_level" = rep(c(1, 2, 2), times = 5),
                         "CW" = scale(c(rep(c(132.2, 128.8, 145, 
                                        95.7, 111.8, 133.7), 
                                      times = 2),
                                  132.2, 128.8, 145), center = TRUE))
...
file_start <- paste0("../output/WGCNA_output/hemat_transcriptomev1.6/amb_vs_elev_no_filter/", variable, "_as_var/")
...
sft <- pickSoftThreshold(CrabExpr0, 
                         powerVector = powers, 
                         verbose = 5, 
                         networkType = "signed")
```
...
Our mean connectivity drops below the hundreds, so we can safely continue.

I chose to use a signed network, since that's generally recommended over unsigned. Therefore, the WGCNA FAQ recommends choosing a soft-thresholding power of 18, since we have fewer than 20 samples total.
...
```{r coexpression}
softPower <- 18
adjacency <- adjacency(CrabExpr0, power = softPower, type = "signed")
...
# Write kME file
fileName <- paste(file_start, "kME_table.txt")
