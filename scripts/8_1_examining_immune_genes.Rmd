...
for (i in 1:length(transcriptomes)) {
  ...
  # Remove species column if present
  if("species" %in% colnames(immune_tab)) {
  ...
  # Remove all columns that aren't Elevated + Ambient, Days 0+2
  immune_tab <- select(immune_tab, -c(id073_EstCts,
                                      id113_EstCts,
                                      id151_EstCts,
                                      id221_EstCts,
                                      id222_EstCts,
    ...
    write.table(immune_tab, file = paste0("../output/kallisto_matrices/", transcriptomes[i], "/immune_genes_indiv_libraries/elev02_amb02_matrix.txt"),
              row.names = FALSE,
              sep = "\t")
}
...
We're also going to quickly examine all cbai_v4.0 Elevated Day 0 vs Elevated Day 2 DEGs with GO terms related to immune response. This will be done by:

- Pulling our table of accession IDs and GO terms for DEGs created in 4_3_uniprot_to_GO.Rmd

- Selecting it for only genes that have a GO term related to immune response
...
This leaves us with exactly 1 gene - P07154 (CATL1_RAT), AKA Procathepsin L. 

Procathepsin L is, as the name implies, a precursor to Cathepsin L
