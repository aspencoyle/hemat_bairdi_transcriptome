...
for (i in 1:length(blast_filepaths)) {
  ...
  if(any(grepl("|", gene_ids, fixed = TRUE))){
    ...
    blast_tab <- blast_tab %>%
      select(Transcript_ID, Accession_ID)
  ...
  immune_tab <- immune_tab %>%
    select(Accession_ID, Transcript_ID)
...
immune_tab <- read.table(file = paste0("../output/immune_genes/", transcriptomes[1], "/transcript_accession_IDs/all_indiv_libraries.txt"),
                         header = TRUE)
...
TPM_tab <- read.table(file = paste0("../output/TPM_counts/", transcriptomes[1], "/all_indiv_library_TPMcts.txt"),
                      header = TRUE,
                      check.names = FALSE)
