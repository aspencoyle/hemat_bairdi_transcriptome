---
title: "5_19_WGCNA_cbai2.0_AmbLowCrabs"
author: "Aidan Coyle"
...
## Analysis of Gene Expression using WGCNA

We will now run WGCNA on all individual libraries 

This will be ran on libraries aligned to cbai_transcriptomev2.0, which is unfiltered by taxa.

In this analysis, we will exclude the elevated-temperature treatment group (crabs G, H, and I). Ordinarily, you want all possible samples in your WGCNA analysis. However, this script is meant to specifically examine the effect of _change_ in qPCR Hematodinium infection level over time. Since we only have Day 0 qPCR data for the elevated-temperature treatment group (and no Day 17), we will drop these 3 samples, leaving us still with 18. This is above the WGCNA minimum of 15.

This will also remove the Shell Condition variable, as only one crab (F) has an old shell condition.
...
libraries <- c("178", "118", "132", "073", "151", "113", "359", "349", "334", "221", "254", "222", "463", "481", "485", "427", "445", "425")

# For each row, crab and day should correspond to the order of libraries (ex: 4th row of crabTraits should match libraries[4])
crabTraits <- data.frame("crab" = binarizeCategoricalVariable(x = c(rep(c("A", "B", "C", "D", "E", "F"), times = 3)),
                         ...
                         "temp" = c(rep("Ambient", times = 9),
                                    ...
                                    rep("Ambient", times = 3),
                                    rep("Lowered", times = 3)),
                         # For this run, using hemat_level on Day 0
                         "hemat_level" = scale(rep(c(283, 316000, 546000,
                                                       761000, 210, 9140), times = 3)),
                         # Also creating variable for difference in Hematodinium level (qPCR
                         # Day 17 SQ mean as percent of first timepoint, on a log scale.
                         "hemat_diff" = scale(rep(c(2.838, -0.8379, 1.341, 
                                                    -3.103, 3.139, -0.4558), times = 3)),
                         "CW" = scale(rep(c(132.2, 128.8, 145, 
                                        127.4, 134.1, 133.3), times = 3), center = TRUE))

# Create clinical data trait matrix. Same rules as above, but all columns are numeric are numeric. Crab A will be noted as 1, B as 2, and C as 3. 


crabClinicalData <- data.frame("crab" = binarizeCategoricalVariable(x = c(rep(c(1, 2, 3, 4, 5, 6), times = 3)),
                                    includePairwise = FALSE,
                                    includeLevelVsAll = TRUE,
                                    minCount = 2),
                         "day" = c(rep(0, times = 6),
...
# Variable being examined - should match a column in two data frames above
variable <- "day"

# Start and ending we want for each file and graph saved
file_start <- paste0("../output/WGCNA_output/cbai_transcriptomev2.0/amb_low/", variable, "_as_var/")
...
Typically, we would choose the lowest power below 30 (for signed networks) that reached an R2 value of 0.8 or higher. However, no eligible soft-thresholding power reached 0.8. According to the [WGCNA FAQ](https://horvath.genetics.ucla.edu/html/CoexpressionNetwork/Rpackages/WGCNA/faq.html), I should choose a soft-thresholding power of 18, since we have under 20 samples.
