---
title: "58_WGCNA_cbai4.0_hematv1.6_AmbCrabs"
author: "Aidan Coyle"
...
We will include transcripts from both C. bairdi and Hematodinium. However, rather than using an unfiltered transcriptome, we will use libraries aligned to a) a transcriptome produced by a BLASTx against the _C. opilio_ genome, and b) a transcriptome filtered to only include Alveolata sequences. These libraries were then appended to each other.

The two transcriptomes used to create the libraries are cbai_transcriptome_v4.0 and hemat_transcriptome_v1.6

Table of libraries included in analysis:
...
All kallisto libraries should be available within the GitHub repo. However, one datafile - our blastx table, cbai4.0_hemat1.6_transcriptomes_blastx_table.txt. This is two BLASTx annotations - of hemat_transcriptome_v1.6.fasta and cbai_transcriptome_v4.0.fasta - appended to each other. 
...
**Description of hemat_transcriptome_v1.6.fasta and BLASTx annotation**
...
kallisto_path <- "../output/kallisto_libraries/cbaiv4.0_hematv1.6_combined/"
...
# Create clinical data trait matrix. Same rules as above, but both crab and day are numeric. Crab G will be noted as 1, H as 2, and I as 3
...
file_start <- "../output/WGCNA_output/cbai4.0_hemat1.6_transcriptomes/amb_crabs_no_filter/"

# Location of blastx table
blastx_table_site <- "../data/cbai4.0_hemat1.6_transcriptomes_blastx_table.txt"
...
for (i in 2:length(kallisto_files)){
  ...
  TPMcounts <- left_join(TPMcounts, kallisto_output, by = "Transcript_ID")
}
```
...
duplicates <- kallisto_output[duplicated(kallisto_output$Transcript_ID), ]
